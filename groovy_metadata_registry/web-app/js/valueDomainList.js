...
$(document).ready(function() {
	...
	$('#valueDomainList').html( '<table cellpadding="0" cellspacing="0" border="0" class="display" id="valueDomainTable"></table>' );
	oTable = $('#valueDomainTable').dataTable( {
        "bProcessing": true,
        "bServerSide": true,
		...
		"aoColumns": [
		    { "mDataProp": "refId", "sTitle":"Ref ID", "sWidth":"10%"},
			{
				...
				"mRender": function ( data, type, row ) {		
					return '<a id="'+ row.id + '" href="/groovy_metadata_registry/valueDomain/show/' + row.id + '">' + data + '</a>'
			    },
			    "mDataProp": "name",
			    "sWidth":"30%",
			    "sTitle":"name"
			},
			{
				...
				"mRender": function ( data, type, row ) {	
					if(data!=null){
						return '<a href="/groovy_metadata_registry/dataType/show/' + row.dataType_id + '">' + data + '</a>'
					}else{
						return ''
					}
			...
			{ "mDataProp": "format", "sTitle":"Format", "sWidth":"10%"},
			{
			    ...
			    // Using `row[0]` is equivalent.
			"mRender": function ( data, type, row ) {		
				return '<a href="/groovy_metadata_registry/conceptualDomain/show/' + row.conceptualDomain_id + '">' + data + '</a>' + '<img class="floatright" src="../images/details_open.png" />'
		    },
	...
	$('#valueDomainTable tbody td img').live( 'click', function () {
