...
class ModelController {
	...
	def dataTables(){
		...
		//if the user searches for a catalogue return the search results using the catalogue service
	 ...
	 * show the catalogue in question using the find instance function and the dataElement service
	 * ...presuming they have the appropriate permissions
	 *************************************************************************************** */
	
	def show(Long id) {
		
		//use the find instance method to get the catalogue in question
		
		def modelInstance = findInstance()
		
		//if you can't find it or don't have permission go back to the list page otherwise show the catalogue
		
		if (!modelInstance) {
			flash.message = message(code: 'default.not.found.message', args: [message(code: 'catalogue.label', default: 'Model'), id])
	 ...
	 * renders the catalogue template so the user can create a catalogue
	 ...
	 * calls the catalogue service to create a catalogue and from the data elements included in the users
	 * catalogue basket. The service sets admin permissions
	 * on that object for the user in question and creates the catalogue
	 *************************************************************************************** */
	...
	def saveBasketModel(){
		...
		//create the catalogue with the parameters passed from the catalogue basket
		
		def modelInstance = modelService.createFromBasket(params)
		
		//if there are any errors in the catalogue put them into a format
		...
		if(modelInstance.errors){
			...
			if(errors.size()>0){
				//redirect to the catalogue basket controller any errors displayed
	 ...
	 * calls the catalogue service to create a catalogue and  sets admin permissions
    ...
    def save() {
		...
		/* ***
		 * validate the catalogue looking at it's schema specifications and ensuring they are mutually exclusive
		 * i.e. a catalogue cannot have a data element that is both mandatory and optional
		 * ****/
	 ...
	 * this function redirects to the edit catalogue screen
	 *********************************************************************************** */
	 ...
	 * this function updates the catalogue using the catalogue service
	 *********************************************************************************** */
    ...
    def update(Long id, Long version) {
		...
		//get the catalogue (redirect if you can't)
		...
		//check that we have the right version i.e. no one else has updated the catalogue whilst we have been
		//looking at it

        if (version != null) {
            if (modelInstance.version > version) {
                modelInstance.errors.rejectValue("version", "default.optimistic.locking.failure",
                          [message(code: 'catalogue.label', default: 'Model')] as Object[],
		...
		//update the catalogue using the catalogue service
	 ...
	 * this function deletes the catalogue using the catalogue service
	 *********************************************************************************** */

    def delete(Long id) {
		
		//get the catalogue and redirect if it doesn't exist
        def modelInstance = findInstance()
        if (!modelInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'catalogue.label', default: 'Model'), id])
            redirect(action: "list")
		...
		//call the catalogue service  to delete the catalogue

        try {
			
			modelService.delete(modelInstance)
			
            flash.message = message(code: 'default.deleted.message', args: [message(code: 'catalogue.label', default: 'Model'), id])
            redirect(action: "list")
        }
        catch (DataIntegrityViolationException e) {
            flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'catalogue.label', default: 'Model'), id])
	 ...
	 * this function removes a data element from a catalogue
	 *********************************************************************************** */
	 ...
	 * this function grant permission to the given catalogue
	 *********************************************************************************** */
	...
	/* **********************************************************************************
	 * this function uses the catalogue service to get the catalogue so that
	...
	/* **********************************************************************************
	 * this function redirects to the show catalogue screen
	 *********************************************************************************** */
	...
	/* **********************************************************************************
	 * this function checks to see if the catalogue passed to it contains errors i.e. when a
	...
	/* **********************************************************************************
	 * this function validates the parameters submitted when creating or updating a catalogue
	 * in particular this validates that the data element schemas are mutually exclusive
	 * i.e. a data element cannot be both mandatory and optional within a given catalogue
	 *********************************************************************************** */
	...
	def validateLinkedDataTypes(){
		...
		if(mandatoryDataElements && requiredDataElements){
			if(catalogueElementService.parameterContains(mandatoryDataElements, requiredDataElements)){
				...
				flash.message = 'Added data elements must either be mandatory or required for any given catalogue, not both'
		...
		if(mandatoryDataElements && optionalDataElements){
			if(catalogueElementService.parameterContains(mandatoryDataElements, optionalDataElements)){
				flash.message = 'Added data elements must either be mandatory or optional for any given catalogue, not both.'
				return false
			}
		}
		...
		if(mandatoryDataElements && referenceDataElements){
			if(catalogueElementService.parameterContains(mandatoryDataElements, referenceDataElements)){
				flash.message = 'Added data elements must either be mandatory or reference for any given catalogue, not both'
				return false
			}
		}
		...
		if(requiredDataElements && optionalDataElements){
			if(catalogueElementService.parameterContains(requiredDataElements, optionalDataElements)){
				flash.message = 'Added data elements must either be required or optional for any given catalogue, not both'
				return false
			}
		}
		...
		if(optionalDataElements && referenceDataElements){
			if(catalogueElementService.parameterContains(optionalDataElements, referenceDataElements)){
				flash.message = 'Added data elements must either be required or reference for any given catalogue, not both'
				return false
			}
		}
