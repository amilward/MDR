...
import uk.co.mdc.ModelBasket
	...
	/* *********************************************************************
	 * This service allows the user to access the catalogue catalogue
	 * It will be called by the catalogue controller and is written with
...
class ModelService {
	...
	@PreAuthorize("hasPermission(#catalogue, admin)")
	@Transactional
	 ...
	 * requires that the authenticated user to have ROLE_USER to create a catalogue
	 ********************************************************************************* */
	...
	Model createFromBasket(Map parameters) { 
		...
		if(modelInstance.save(flush:true)){
			...
			//return the catalogue to the consumer (the controller)
	...
	@PreAuthorize('hasPermission(#id, "uk.co.mdc.catalogue.Model", read) or hasPermission(#id, "uk.co.mdc.catalogue.Model", admin)')
	Model get(long id) {
	   Model.get id
	   }
	...
	/* ************************* UPDATE ModelS***********************************************
	 *  requires that the authenticated user have write or admin permission on the catalogue instance to edit it
	 ******************************************************************************************** */
